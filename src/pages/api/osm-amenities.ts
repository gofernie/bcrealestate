...
export const GET: APIRoute = async ({ url }) => {
  ...
  function normalizeOsmImage(
    ...
    const commons = String(
      row.wikimedia_commons || ""
    ).trim();

    if (commons) {
      return `/api/commons-image?file=${encodeURIComponent(
        commons
      )}`;
    }
    ...
    if (wikidata) {
      return `/api/wikidata-image?id=${encodeURIComponent(
        wikidata
      )}`;
    }
